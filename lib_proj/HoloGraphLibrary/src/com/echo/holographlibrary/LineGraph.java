...
public class LineGraph extends View {
	...
	/**
	 * 기존 라이브러리는 입력된 값의 최대, 최소를 기준으로 그리기때문에 x축이 동적으로 할당됨. 혈당관리 서비스앱에서는 x축이 일, 주,
	 * 월, 년 으로 고정이기 때문에 x축 최대값을 별도로 설정.
	 * 
	 * @author leejeongho
	 * @since 2014.09.03
	 * @param x
	...
	public float getMaxX() {
		/*
		 * float max = lines.get(0).getPoint(0).getX(); for (Line line : lines){
		 * for (LinePoint point : line.getPoints()){ if (point.getX() > max) max
		 * = point.getX(); } } maxX = max;
		 */
		return maxX;
	}
	...
	/**
	 * x축의 시작 범위. x축의 시작 범위를 변경하고 싶을때 설정.
	 * 
	 * @param x
	 *            x축의 최소값.
	 */
	...
	public float getMinX() {
		 ...
		 * float max = lines.get(0).getPoint(0).getX(); for (Line line : lines){
		 * for (LinePoint point : line.getPoints()){ if (point.getX() < max) max
		 * = point.getX(); } } maxX = max; return maxX;
		 */
		return minX;
	}
		...
		if (fullImage == null || shouldUpdate) {
			...
			for (Line line : lines) {
				...
				if (lineCount == lineToFill) {
					...
					path.lineTo(getWidth() - sidePadding, getHeight()
							- bottomPadding);
			...
			canvas.drawLine(sidePadding, getHeight() - bottomPadding,
					getWidth(), getHeight() - bottomPadding, paint);
			...
			if (this.showHorizontalGrid) {
				canvas.drawLine(sidePadding, (getHeight() - bottomPadding) / 2,
						getWidth(), (getHeight() - bottomPadding) / 2, paint);
				canvas.drawLine(sidePadding, bottomPadding, getWidth(),
						bottomPadding, paint);
			}
			/*
			 * for(int i=1;i<=10;i++) { canvas.drawLine(sidePadding, getHeight()
			 * - bottomPadding-(i*lineSpace), getWidth(),
			 * getHeight()-bottomPadding-(i*lineSpace), paint); }
			 */
			paint.setAlpha(255);
			...
			for (Line line : lines) {
				...
				if (line.isShowingPoints()) {
					for (LinePoint p : line.getPoints()) {
						...
						/**
						 * 포인트의 색상을 line 색과 동일하게 표현되도록 변경. 혈압일 경우 두번째 포인트의 색상을
						 * 변경.
						 * 
						 * @author leejeongho
						 * @since 2014.09.03
			...
			if (this.showMinAndMax) {
				...
				int half = ((max_i - min_i) / 2) + min_i;
				ca.drawText(max, 0, txtPaint.getTextSize(), txtPaint);
				ca.drawText(String.valueOf(half), 0, this.getHeight() / 2,
						txtPaint);
